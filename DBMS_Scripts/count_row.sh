...
select count in "all" "column"; do
    ...
    all)
        ...
        clear
        echo -e "${BBlue}\n\n\n\n\n\n\n\n\n\n\t\t\t\t\t\t==============================${NC}"
        echo -e "${BBlue}\t\t\t\t\t\t|${NC}      ${BWhite}Number of rows is $result${NC}  ${BBlue}|${NC}"
        echo -e "${BBlue}\t\t\t\t\t\t==============================${NC}\n\n"
        echo -e "${BYellow}\t\t\t\t\t\t${BWhite}Back To Table Contol Menu${NC} ......${NC}\c"
        read press
        #######
...
if [ $column -eq 1 ]; then
    ################################################## error###############
    echo -e "\n\t\t\t\t\t\t${BYellow}Which column you want to count? Choose from 1 to $columns_length:  ${NC}\c"
    read -r col_pos

    if ! [[ "$col_pos" =~ ^[0-9]+$ ]]; then
        clear
        echo -e "${BBlue}\n\n\n\n\n\n\n\n\n\n\t\t\t\t\t\t==============================${NC}"
        echo -e "${BBlue}\t\t\t\t\t\t|${NC}      ${BWhite}PLease enter valid value${NC}   ${BBlue}|${NC}"
        echo -e "${BBlue}\t\t\t\t\t\t==============================${NC}\n\n"
        echo -e "${BYellow}\t\t\t\t\t\t${BWhite}Back To Table Contol Menu${NC} ......${NC}\c"
        read press
        Table_Menu
    ...
    clear
    echo -e "${BBlue}\n\n\n\n\n\n\n\n\n\n\t\t\t\t\t\t==============================${NC}"
    echo -e "${BBlue}\t\t\t\t\t\t|${NC}      ${BWhite}Number of filled fields in the column is $counter${NC}  ${BBlue}|${NC}"
    echo -e "${BBlue}\t\t\t\t\t\t==============================${NC}\n\n"
    echo -e "${BYellow}\t\t\t\t\t\t${BWhite}Back To Table Contol Menu${NC} ......${NC}\c"
    read press
    Table_Menu
